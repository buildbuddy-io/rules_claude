"""Module extensions for rules_claude (bzlmod)."""
...
_TOOLCHAIN_BUILD_HEADER = """# Generated by rules_claude

load("@rules_claude//claude/private:toolchain.bzl", "claude_toolchain")
...
toolchain(
    ...
    toolchain_type = "@rules_claude//claude:toolchain_type",
)
...
toolchain(
    ...
    toolchain = ":{platform}_impl",
    toolchain_type = "@rules_claude//claude:runtime_toolchain_type",
)
"""
...
def _find_modules(module_ctx):
    """Find the root module and rules_claude module.
    ...
    for mod in module_ctx.modules:
        if mod.is_root:
            root = mod
        if mod.name == "rules_claude":
            rules_claude = mod
    if root == None:
        root = rules_claude
    if rules_claude == None:
        fail("Unable to find rules_claude module")
    return root, rules_claude

def _claude_impl(module_ctx):
    """Implementation of the claude module extension."""
    root, rules_claude = _find_modules(module_ctx)

    downloads = root.tags.download or rules_claude.tags.download
